...
class BriefFluentTQL2Eng {
	...
	def String singleTaintFlows(TaintFlow taintFlow) {
		var FlowParticipant source = taintFlow.getFrom().get(0)
		var FlowParticipant sink = taintFlow.getTo().get(0)
		...
		if(source instanceof Method)
			taintFlowInEnglish += "from the source Method: " + (source as Method).getName()
		else 
			taintFlowInEnglish += "one of the source in the MethodSet: " + (source as MethodSet).getName()
			
		if(through.size > 0) {
			if(through.get(0) instanceof Method)
			taintFlowInEnglish += " and does not go through the sanitizer Method: " + (through.get(0) as Method).getName()
		else
		...
		for (i : 0 ..< notThrough.size()) {
			if(notThrough.get(i) instanceof Method)
				taintFlowInEnglish += " and goes through the required propogator Method: " + (notThrough.get(0) as Method).getName()
			else
		...
		if(sink instanceof Method)
			taintFlowInEnglish += " and finally reaches the sink Method: " + (sink as Method).getName()
		else 
	...
	def String translate(TaintFlowQuery taintFlowQuery) {
		...
		var int reportLocation = taintFlowQuery.getReportLocation()
		
		if(reportLocation == 1)
			fluentTQL2E += "the source location in the IDE."
		else if(reportLocation == 2)
			fluentTQL2E += "the sink location in the IDE."
		else if(reportLocation == 3)
